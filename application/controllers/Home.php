...
class Home extends CI_Controller
{
    ...
    public function review_submit()
    {
        if (count($_POST) > 0) {
            ...
            if ($insert) {
                $this->session->set_userdata('rmsg', '<p class="alert alert-success"> Your review has been submitted successfully. </p>');
            } else {
                $this->session->set_userdata('rmsg', '<p class="alert alert-danger"> We are facing technical error ,please try again later or get in touch with Email ID provided in contact section.</p>');
            }
            redirect($_SERVER['HTTP_REFERER']);
        }
    }
    ...
    {
        if ($this->input->get('company-name') || $this->input->get('company-location') != '') {
            $searchname = $this->input->get('company-name');
            $searchlocation = $this->input->get('company-location');

            $serch_sql = "SELECT *  FROM company
            LEFT JOIN company_subcategory ON company.company_subcategory = company_subcategory.category_id
            WHERE company.company_name LIKE '%{$searchname}%' OR company.company_subcategory LIKE '%{$searchname}%' ";
...
(Ekaum)';
    ...
    public function banner_update()
    {
        if (count($_POST) > 0) {
            ...
            if ($_FILES['company_banner']['name'] != '') {
                $img = imageUpload('company_banner', 'uploads/company/');
                $post['company_banner'] = $img;
                if ($qr != "") {
                    $img = imageUpload('company_banner', 'uploads/company/');
                }
            } else {
                $img = $post['company_banner'];
            }
    ...
    public function choose_vcard()
    {
        ...
        if (count($_POST) > 0) {
            ...
            redirect(base_url('sahar/' . $get_company[0]['company_city'] . '/' . url_title(strtolower($get_category[0]['category'])) . '/' . $get_company[0]['company_web_title']));
    ...
    public function bank_details()
    {
        ...
        if (count($_POST) > 0) {
            $post = $this->input->post();
            $post['company_id'] = sessionId('login_user_id');
            $counts = $this->CommonModal->getNumRows('bank_details', 'company_id', $this->session->userdata('login_user_id'));
