							...
							<div class="tab-pane fade show active" id="placesseach" role="tabpanel" aria-labelledby="placesseach-tab">
								<form class="main-search-wrap fl-wrap" action="<?= base_url('search') ?>" method="GET">
									<div class="main-search-item">
										...
										<input type="text" class="form-control radius" placeholder="What are you looking for?" name="company-name" />
									</div>
									<div class="main-search-item">
										<span class="search-tag"><i class="lni lni-map-marker"></i></span>
										<input type="text" class="form-control" placeholder="Location" name="company-location" />
									</div>
												...
												<div class="Goodup-grid-wrap">
													<div class="Goodup-grid-upper">
														...
														<div class="Goodup-grid-thumb">
															<a href="<?= base_url() ?>listing/<?= url_title($serv['company_city']) ?>/<?= url_title(strtolower($cat_tag[0]['category'])); ?>/<?= url_title($serv['company_name']) ?>/<?= encryptId($serv['company_id']) ?>" class="d-block text-center m-auto"><img src="uploads/company/<?= $serv['company_banner'] ?>" class="img-fluid" alt=""></a>
														</div>

													</div>
													<div class="Goodup-grid-fl-wrap">
														<div class="Goodup-caption px-3 py-2">
															<div class="Goodup-author"><a href="author-detail.html"><img src="uploads/company/<?= $serv['company_logo'] ?>" class="img-fluid circle" alt=""></a></div>
															<div class="Goodup-cates multi"><a href="" class="cats-1"><?= $cat_tag[0]['category'] ?></a><a href="" class="cats-2"><?= $sub_tag[0]['subcategory'] ?></a></div>
															<h4 class="mb-0 ft-medium medium"><a href="<?= base_url() ?>listing/<?= url_title($serv['company_city']) ?>/<?= url_title(strtolower($cat_tag[0]['category'])); ?>/<?= url_title($serv['company_name']) ?>/<?= encryptId($serv['company_id']) ?>" class="text-dark fs-md"><?php echo $serv['company_name']; ?></a></h4>
															<div class="Goodup-location"><i class="fas fa-map-marker-alt me-1 theme-cl"></i><?= $serv['company_city'] ?>, <?= $state[0]['state_name'] ?></div>
															<div class="Goodup-middle-caption mt-3">
															    <?php 
															    if($serv['company_about'] != ''){
															        ?>
																<?= substr($serv['company_about'], 0, 50) ?>...
																<?php
															    }
															    ?>
															</div>
														...
														<div class="Goodup-grid-footer py-2 px-3">
															<div class="Goodup-ft-first">
																<a href="<?= base_url() ?>listing/<?= url_title($serv['company_city']) ?>/<?= url_title(strtolower($cat_tag[0]['category'])); ?>/<?= url_title($serv['company_name']) ?>/<?= encryptId($serv['company_id']) ?>" class="Goodup-cats-wrap">
															...
															<div class="Goodup-ft-last">
																<div class="Goodup-inline">
                                                                    <!--<a href="<?= base_url() ?>listing/<?= url_title($serv['company_city']) ?>/<?= url_title(strtolower($cat_tag[0]['category'])); ?>/<?= url_title($serv['company_name']) ?>/<?= encryptId($serv['company_id']) ?>">-->
